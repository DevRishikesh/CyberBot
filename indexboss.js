...
// 🔥 Message Splitter for long AI responses (Bypasses WhatsApp character limits)
...
- Do not be overly short.
        ...
        const response = await axios.post(API_URL, payload, {
            ...
            timeout: 30000 // Deep research takes time
        });

        let reply = response.data?.candidates?.[0]?.content?.parts?.[0]?.text || "No response from Deep Brain.";

        // 🚀 We now use the splitMessage function to safely bypass WhatsApp limits!
...
daily.messages += 1;
    ...
    if (user.level > oldLevel) {
        await msg.reply(`🎉 LEVEL UP! @${userId.split("@")[0]} is now Level ${user.level} 🔥`);
    }
...
async function summarizeArticle(title, link) {
    ...
    try {
        ...
        const prompt = `
Summarize this cybersecurity news in ONE simple sentence.
...
`;
        ...
        const response = await axios.post(
            "https://api.groq.com/openai/v1/chat/completions",
            {
                model: MODEL,
                messages: [
                    { role: "system", content: "You explain cybersecurity news in simple words." },
